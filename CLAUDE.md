...
**Logistics page** (`/logistics`): 12 sections focused on transport bot product. All external Telegram bot CTAs are computed with `getLogisticsTelegramUrl(Astro.url.searchParams)`. Internal guide links use `getLogisticsGuideUrl` to preserve only the exact approved campaign tuple. Pricing and trial messaging are intentionally absent until separately approved. Real flow: registration/company data → PDFs from entered data → separate XML file for review and download, then manual SEF upload. Bot section (Section 9) has: video + CMR screenshot grid → bot card → feature grid with guide links. CMR screenshot needs `bg-white` container (dark theme readability).
